...
//Méthode CRUD pour ajouter des jeux sur la bibliothèque epic

//Création de données, fonction qui ajoute une entrée au tableau epic
//POST
exports.createData = ( request, response) => {
    ...
    fs.readFile ( path, ( err, data ) => {
        ...
        if (err) {
            ...
            let id;
            //On enregistre les id du tableau epic dans un tableau
            let tabId = dataExist.epic.map( (obj) => obj.id ); 
            ...
            !dataExist.epic.length ? id = 0 : id = Math.max(...tabId)
            //On envoie dans le tableau désigné l'information de la requête à la suite dans la cible
            dataExist.epic.push({ "id" : id +1, "name": request.body.name, "value": request.body.value })
...
//Read
//Lire toutes les entrées du tableau epic
exports.readData = (request, response) => {
    ...
    fs.readFile( path, (err, data) => {
        ...
        //Sinon on affiche la bibliothèque de jeu Epic dans son intégralité
        } else {
            response.status(200).json(JSON.parse(data).epic)
        };
    })
}

//Read 
//lecture d'une entrée de la liste epic par son id
exports.readDataById = (request, response) => {
    ...
    fs.readFile(path, (err, data) => {
        ...
        if(err){
            ...
            //On stocke dans la variable dataExistant les données du tableau epic
            const dataExistant=JSON.parse(data).epic.find(
...
exports.updateData=(request, response) => {
    ...
    fs.readFile(path, (err, data) => { 
        if(err){
            ...
            //dans les données on cherche l'élément name des objets du tableau et on les compare avec l'élément name entrée en input
            const dataId = dataExistant.epic.find(
                (obj) => obj.id === parseInt(request.params.id)
            )
            if(!dataId){
                response.status(404).json({
    ...
    fs.readFile(path, (err, data) => {
        if(err){
            ...
            if(!dataId){
                ...
                dataExistant.epic = dataExistant.epic.filter(
                    (obj) => obj.id != parseInt(request.params.id));
                fs.writeFile( path, JSON.stringify(dataExistant), (writeErr)=> {
                    if(writeErr){
                        response.status(500).json({
